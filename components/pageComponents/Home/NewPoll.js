...
export default function NewPoll() {
  ...
  const handleSubmit = async (e) => {
    ...
    formVal["question"] = document.getElementById("question").value;
    formVal["topic"] = document.getElementById("topic").value;
    ...
    if (errors.length === 0) {
      try {
        ...
        updateAppMessages([{ message: "Poll Created successfully" }]);
        $("#newPollModal").modal("hide"); //closes modal programitically
        clearForm();
      } catch (err) {
        setFormErrors([err]);
      }
  ...
  const clearForm = () => {
    setFormErrors([]);
    document.getElementById("newPoll").reset();
  };
      ...
      <div className="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div className="modal-content">
          ...
          <div className="modal-body">
            {/* <form onSubmit={handleSubmit} id="newPoll"> */}
            <form id="newPoll">
