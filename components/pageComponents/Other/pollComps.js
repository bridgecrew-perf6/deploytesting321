...
import {
  ...
  customPrevArrow,
  customDots,
} from "../../../appStyles/carouselStyles.module.css";
...
export const PollWindow = () => {
  const { data, error } = useQuery(GET_POLLS_ALL);
...
const SliderArrow = (props) => {
  ...
  if (type === "left") {
    pollArrow = <IoIosArrowBack style={{ color: "gray" }} size="25px" />;
    arrowClassName = customPrevArrow;
  } else {
    pollArrow = <IoIosArrowForward style={{ color: "gray" }} size="25px" />;
